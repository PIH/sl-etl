...
-- update index asc/desc on mch_pnc_encounter table
drop table if exists #derived_indexes;
select  encounter_id,
        ROW_NUMBER() over (PARTITION by emr_id order by encounter_datetime, encounter_id) as index_asc,
        ROW_NUMBER() over (PARTITION by emr_id order by encounter_datetime DESC, encounter_id DESC) as index_desc
...
set t.index_asc = i.index_asc,
    t.index_desc = i.index_desc
from mch_pnc_encounter t inner join #derived_indexes i on i.encounter_id = t.encounter_id
;
...
-- update index asc/desc on mch_newborn_progress_discharge_encounter table
drop table if exists #derived_indexes;
select  encounter_id,
        ROW_NUMBER() over (PARTITION by emr_id order by encounter_datetime, encounter_id) as index_asc,
        ROW_NUMBER() over (PARTITION by emr_id order by encounter_datetime DESC, encounter_id DESC) as index_desc
into    #derived_indexes
from    mch_newborn_progress_discharge_encounter;

update t
set t.index_asc = i.index_asc,
    t.index_desc = i.index_desc
from mch_newborn_progress_discharge_encounter t inner join #derived_indexes i on i.encounter_id = t.encounter_id
;
...
-- update patient  index asc/desc on all_medication_dispensing table
drop table if exists #derived_indexes;
select  dispensing_id,
        ROW_NUMBER() over (PARTITION by patient_id order by encounter_datetime, dispensing_id) as index_asc,
        ROW_NUMBER() over (PARTITION by patient_id order by encounter_datetime DESC, dispensing_id DESC) as index_desc
into    #derived_indexes
...
set t.index_asc = i.index_asc,
    t.index_desc = i.index_desc
from all_medication_dispensing t inner join #derived_indexes i on i.dispensing_id = t.dispensing_id
;
...
drop table if exists #derived_indexes;
select  obs_id,
        ROW_NUMBER() over (PARTITION by patient_id order by encounter_datetime, obs_id) as index_asc,
        ROW_NUMBER() over (PARTITION by patient_id order by encounter_datetime DESC, obs_id DESC) as index_desc
into    #derived_indexes
from    ncd_diagnoses;
...
set t.index_asc = i.index_asc,
    t.index_desc = i.index_desc
from ncd_diagnoses t inner join #derived_indexes i on i.obs_id = t.obs_id
;
