...
INNER JOIN temp_obs o ON o.obs_group_id = tgt.obs_group_id and o.concept_id = concept_from_mapping('PIH','1282')
SET
    tgt.order_drug = concept_name(o.value_coded, 'en'),
    tgt.order_formulation = drugName(o.value_drug),
    tgt.product_code = openboxesCode(o.value_drug)
;

UPDATE all_medication_prescribed tgt
INNER JOIN temp_obs o ON o.obs_group_id = tgt.obs_group_id
and o.concept_id = concept_from_mapping('PIH','10744')
SET order_dose_unit = concept_name(o.value_coded, 'en'); -- 1 MINUTE
...
INSERT INTO all_medication_prescribed(order_type, patient_id, encounter_id, visit_id, order_id, order_created_date,
                                      order_date_activated, user_entered, prescriber,
                                      order_drug, order_formulation, order_formulation_non_coded, product_code,
                                      ...
                                      order_frequency, order_duration, order_duration_units, order_reason, order_comments)
SELECT
...
drugName(do.drug_inventory_id) AS order_formulation,
do.drug_non_coded as order_formulation_non_coded,
